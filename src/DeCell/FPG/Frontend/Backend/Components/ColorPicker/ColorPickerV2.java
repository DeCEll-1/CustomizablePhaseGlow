...
import DeCell.FPG.Frontend.Backend.Renderable.ShaderRenderable;
...
public class ColorPickerV2 extends UIContainer<ColorPickerV2, UIComponentAPI> {
    ...
    public final static Rect containerRect = new Rect(0, 0, colorPickerPanelRect.w,
            colorPickerPanelRect.h + yPadding + colorPickerSliderRect.h + yPadding +
                    ((NudRect.h + yPadding) * 3) - yPadding // 3 pairs - 1 ypadding
    );
    ...
    // this is a more extensible color picker that will be more "vague"
    // it will use xyzw, as this color picker will use an outside lambda for calculating the color
    // that will be provided
    // now, it needs 4 sliders
    // 4 numeric up downs for each slider
    // 1 color picker square (yes it is a square because i wanna use shaders to display the input)
    // 1 preview panel
    // gonna go for windowses color picker look ngl
