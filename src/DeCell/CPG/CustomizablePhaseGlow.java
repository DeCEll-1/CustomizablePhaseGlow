...
public class CustomizablePhaseGlow {
    ...
    public static String getKeyFromShip(ShipAPI ship) {
        return CPGPrefix + ShipPrefix + ship.getFleetMemberId();
    }
...
//            Log("Got ship property of " + getKeyFromShip(ship) + " with " + prop);
    ...
    public static void Log(String s) {
        Global.getLogger(CustomizablePhaseGlow.class).log(Priority.INFO, s);
    }

    public static void LogWarn(String s) {
        Global.getLogger(CustomizablePhaseGlow.class).log(Priority.WARN, s);
    }

    public static void LogErr(String s) {
        Global.getLogger(CustomizablePhaseGlow.class).log(Priority.ERROR, s);
    }
        ...
        try {
            ...
            for (ModSpecAPI modSpecAPI : Global.getSettings().getModManager().getAvailableModsCopy()) {
                try {
                    String modJson = "";
                    try {
                        modJson = Global.getSettings().loadText("data/shaders/cpg/PhaseShaders.json", modSpecAPI.getId());
            ...
            for (int i = 0; i < masterArray.length(); i++) {
                ...
                if (zaza.fragmentShaderPath.startsWith("./"))
                    zaza.fragmentShaderPath = zaza.fragmentShaderPath.replace("./", "data/shaders/cpg/");
                if (zaza.vertexShaderPath.startsWith("./"))
                    zaza.vertexShaderPath = zaza.vertexShaderPath.replace("./", "data/shaders/cpg/");
